...
These pin the failure mode that motivated the gate (in the reference
autolens_assistant) — stale symbols like `af.PriorModel` and dead module paths like
`autofit.non_linear.nest`, run from memory via `python3 -c "..."` against a stack where
the symbols had been renamed/removed — plus a current-correct call
(`aplt.corner_cornerpy`) that must NOT be flagged.
...
runs under (source `activate.sh` first). Each case imports autofit, so the file is slow
...
def test_validator_flags_stale_model():
    proc = _run_validator("--code", STALE_MODEL)
    assert proc.returncode == 2
    assert "PriorModel" in proc.stderr


def test_validator_flags_stale_nest_module():
    proc = _run_validator("--code", STALE_NEST)
    assert proc.returncode == 2
...
def test_validator_file_mode(tmp_path):
    ...
    script.write_text(f"{STALE_MODEL}\n", encoding="utf-8")
...
def test_hook_denies_stale_snippet():
    proc = _run_hook(f"{sys.executable} -c '{STALE_MODEL}'")
    assert _decision(proc) == "deny"
    assert "PriorModel" in proc.stdout


def test_hook_denies_stale_nest():
    proc = _run_hook(f'{sys.executable} -c "{STALE_NEST}"')
    assert _decision(proc) == "deny"
...
def test_hook_allows_non_python_command_without_import():
    # Pre-screen must short-circuit (no autofit import) — assert both allow and speed.
...
def test_hook_allows_non_python_count_option_with_pyauto_pattern(tmp_path):
    ...
    script.write_text("af.PriorModel\n", encoding="utf-8")

    proc = _run_hook(f"grep -c af.PriorModel {script}")

    assert _decision(proc) is None


def test_hook_escape_hatch():
    proc = _run_hook(f"{sys.executable} -c '{STALE_MODEL}'", env_extra={"PYAUTO_SKIP_API_GATE": "1"})
    assert _decision(proc) is None
...
def test_hook_escape_hatch_command_prefix():
    ...
    # command text.
    proc = _run_hook(f"PYAUTO_SKIP_API_GATE=1 {sys.executable} -c '{STALE_MODEL}'")
    assert _decision(proc) is None


def test_hook_validates_py_file_argument(tmp_path):
    good = tmp_path / "good.py"
    good.write_text(f"{GOOD}\n", encoding="utf-8")
    assert _decision(_run_hook(f"{sys.executable} {good}")) is None

    bad = tmp_path / "bad.py"
    bad.write_text(f"{STALE_MODEL}\n", encoding="utf-8")
...
def test_validate_source_reports_broken_stack_once_not_stale():
    ...
    mod._module_cache["autofit"] = None
    mod._import_errors["autofit"] = wall

    n_stale, lines, import_failed = mod.validate_source("af.Model; af.Collection; af.DynestyStatic")

    assert n_stale == 0 and lines == []  # broken stack is never "stale symbols"
    assert list(import_failed) == ["autofit"]  # reported once, not per symbol
    assert len(import_failed["autofit"]) <= 201  # truncated, not the full wall
