...
the standard aliases (`af`, `aplt` and the bare module names),
...
This is the mechanical half of the `af_audit_skill_apis` skill. The skill drives
...
# ---------------------------------------------------------------------------
# API baseline. autofit_assistant is tied to an autofit version; this file
...
# Modules whose public surface we hash. The two library roots carry
...
# length-descending so the regex alternation prefers `autofit` over `af`.
# ---------------------------------------------------------------------------
...
def _cross_module_candidates(leaf: str) -> list[str]:
    """Look for `leaf` anywhere across the PyAuto* roots + one submodule level."""
...
def select_files(root: Path, scope: str) -> list[Path]:
    ...
    # recursively because scripts/ mirrors autofit_workspace's nested layout.
...
def _installation_kind(module_locations: dict[str, str]) -> str:
    ...
    try:
        distribution = importlib.metadata.distribution("autofit")
    ...
    location = module_locations.get("autofit", "")
...
def render_installation_check(check: InstallationCheck) -> str:
    ...
    if check.locations.get("autofit"):
        lines.append(f"  autofit: {check.locations['autofit']}")
    ...
    if check.status != "ready":
        lines.extend(
            [
                ...
                "  Otherwise use `skills/af_setup_environment.md`; do not install into an",
...
def compute_baseline() -> dict:
    ...
    for name in BASELINE_MODULES:
        mod = importlib.import_module(
            name
        )  # already known importable (plot via autofit)
        names = _public_names(mod)
        api_surface[name] = {"hash": _api_hash(mod), "n_symbols": len(names)}

    return {
        "_comment": "API baseline for autofit_assistant - see autoassistant/audit_skill_apis.py "
        "and skills/af_audit_skill_apis.md. Regenerate with --write-baseline.",
...
def validate_source(text: str) -> tuple[int, list[str], dict[str, str]]:
    ...
    `af.PriorModel` or a removed `aplt` plotter actually crashes. Returns
...
def run_code_gate(*, code: str | None, file: str | None) -> int:
    ...
    if import_failed:
        ...
        print(
            "  Fix the environment first (skills/af_setup_environment.md); if this is "
