...
Two recurring failures motivated this hook (in the reference autolens_assistant): a
session generated stale plotter and import paths from memory and ran them
...
The same script backs both the autofit_assistant hook and the PyAutoLabs-monorepo hook;
...
# Alias-rooted symbol (`af.`, `aplt.`, `autofit.` …).
...
def main() -> None:
    ...
    if reports:
        _deny(
            ...
            "(these are the PriorModel-style stale-from-memory errors).\n\n"
            + "\n\n".join(reports)
