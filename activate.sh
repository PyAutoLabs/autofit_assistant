...
#
# Most users just `pip install autofit` (the environment-setup skill walks through it)
# into a virtual environment. By default this looks for a `.venv` created inside the
# project; edit VENV to point elsewhere, e.g. ~/venv/PyAuto.
#
# Resolving relative to this file (not the current directory) means it works both for a
# local `source activate.sh` and for HPC scripts' `source $PROJECT_PATH/activate.sh`.
