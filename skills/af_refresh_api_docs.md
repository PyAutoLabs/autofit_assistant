...
This is deliberately broader than [`af_audit_skill_apis`](./af_audit_skill_apis.md)
and [`af_update_wiki`](./af_update_wiki.md), not a replacement for either. The audit
...
[`af_setup_environment.md`](./af_setup_environment.md). A refresh pass against a broken
...
wiki-source diff. If the misses touch `wiki/core/`, keep the audit report open for the
next pass.

See [`af_audit_skill_apis.md`](./af_audit_skill_apis.md) for the detailed fix loop.
...
[`af_update_wiki.md`](./af_update_wiki.md):
...
- When the user only wants one known-broken symbol fixed. Use
  [`af_audit_skill_apis.md`](./af_audit_skill_apis.md) directly.
- When the user already knows the exact wiki page whose pinned source moved. Use
  [`af_update_wiki.md`](./af_update_wiki.md) directly.
