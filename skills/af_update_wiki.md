...
The wiki is the content layer of the workspace — every `wiki/core/` page documents one
slice of the PyAuto\* API or the statistics it implements (model composition, priors,
non-linear searches, graphical models, configuration, etc.). Each page pins the source
commits it derives from, so we can detect when it's gone stale. This skill walks every
page, diffs its pinned sources against current HEAD, and rewrites the sections that have
drifted.
...
- *"Refresh against `main` of both repos, or just the one whose API moved?"*
...
A targeted refresh ("PyAutoFit only, since a Samples class was renamed") beats a sweep
("rebuild everything"). Sweeps are for the initial wiki seed; for ongoing maintenance,
narrow.
...
- If installed (`python -c "import autofit, pathlib, inspect; print(pathlib.Path(inspect.getfile(autofit)).parent)"` works), read from the install location.
...
[`af_audit_skill_apis.md`](./af_audit_skill_apis.md) "Provenance"). Do this only after the
...
and the workspace example scripts — never from changelogs or release notes (see
...
- If it doesn't, list it to the user at the end of the run:
  > "I noticed `PyAutoFit` exported a new search class. It probably wants a row in
  > `wiki/core/concepts/non_linear_search.md`; want me to draft one?"
...
2. Run the verification scripts in any skill whose wiki page changed (e.g. if you
   rewrote the searches page, smoke-test `af_run_search`'s recipe).
...
- **`wiki/literature/` is not this skill's surface** — it derives from papers, not
  source code; extend it via `af_ingest_paper`.
