...
Skills cite a lot of PyAuto\* symbols — class names, search algorithms, priors, the
plot API — and the wiki cites more, in API tables and prose. Both can drift silently
when the underlying libraries rename, move, or remove things. `af_update_wiki`
...
Like `af_update_wiki`, the philosophy is **curate, don't auto-rewrite**. The script
...
class might map to a different object entirely (e.g. the old `PriorModel` becoming
`af.Model` with different composition semantics), and the script's suggestions are
heuristic. The audit answers *what's broken*; the agent + user answer *what's right*.

Before starting, confirm with the user:

- Which scope: **skills only**, **wiki only**, or **all**.
...
[`af_setup_environment.md`](./af_setup_environment.md). Don't audit against a
...
versions, loaded `autofit` path, and likely install type. Route exits 2/3 to
`af_setup_environment`; only a subsequent `--check-version` exit 1 is API drift.
...
(default) covers skills + wiki/core + generated `scripts/` `.py` files.
The **scripts scope** is what catches stale symbols in generated pipelines and exploration
scripts — the place an old removed PyAutoFit symbol actually executes — and it skips this
repo's own committed tooling (`audit_skill_apis.py`, `refresh_api_docs.py`). The report
lands at `autoassistant/audit/skill_api_audit_<YYYY-MM-DD>.md` and is **gitignored** —
only the script is committed. Exit code is non-zero when the report contains misses; you
can chain `python autoassistant/audit_skill_apis.py && echo clean || echo drift` in shell
loops.
...
(`af_update_wiki` / `af_refresh_api_docs`) pulls from upstream source, follow either with
...
The symbol resolver above only checks *alias-rooted dotted symbols* (`af.Model`).
It is structurally blind to a defunct **operator idiom** — one where every named symbol
still imports but the *construction* was removed. The canonical case is analysis-summing:
the old multi-dataset combine joined analyses with `+` (or folded a list with `sum(...)`)
to sum their log-likelihoods. Every token still resolves, so `--scope all` and the code
gate report it clean — yet the `+` overload is gone, and the current combine is the factor
graph (`af.AnalysisFactor` wrapped per dataset, combined with `af.FactorGraphModel`, run
via `search.fit(model=factor_graph.global_prior_model, analysis=factor_graph)`).

The deny-list (`DENY_LIST` in `autoassistant/audit_skill_apis.py`) closes that gap. Each
entry is `{regex, why_defunct, replacement, citation}`. Scan the docs for hits:

```bash
python autoassistant/audit_skill_apis.py --lint-idioms   # 0 clean, 1 on any hit
```

It scans `skills/` + `wiki/` (all of it, including `concepts/`) + `scripts/`, is
self-contained (no installed stack needed), and is also run inside the code gate, so a
snippet or `.py` file the agent is about to execute is checked for these idioms too.
**Add an entry whenever a fix is "rewrite the construction", not "rename the symbol"** —
the two audits are complementary. A file of intentional fixtures opts out with the
`pyauto-api-gate: skip` marker.
...
Neither verifies a `wiki/core` page's `pinned_commit` is honest — and (in the reference
autolens_assistant) drift once shipped exactly because a pin was bumped without
re-validating content. Provenance closes that with two signals:
...
honest re-pin, the partner of `af_update_wiki` step 4):

```bash
python autoassistant/audit_skill_apis.py --write-provenance --page wiki/core/concepts/<page>.md
```

### 6. Citation paths — the layout axis the symbol audit is blind to

A page can cite only live `af.*` symbols while its `` `Project:relative/path` ``
source citations point at a pre-refactor file layout (module → package moves like
`nest/nautilus.py` → `nest/nautilus/`, deleted files, renamed READMEs). The two are
independent failure axes: a tree can be 0-broken-symbols and still carry dozens of
stale paths.
...
(`af_refresh_api_docs`) and before a release.

ERRORs fail the check; warnings (unpinned `main`, unstamped legacy pages) do not unless
`--strict`, so the release/PR check goes red on genuine forgery/staleness without noise
from legacy `main`-pinned pages that predate the discipline.
...
references a PyAuto* symbol absent from the *installed* stack. Run the same validator by
hand on a snippet or a file:

```bash
python autoassistant/audit_skill_apis.py --code "import autofit as af; af.PriorModel"  # exit 2
...
python -c "import autofit as af; help(af.Model)" | head -40
...
for name in dir(af):
    obj = getattr(af, name, None)
    if obj is not None and 'Prior' in name:
        print(name, '->', obj)
"
```
...
Replace the symbol in place. If the rename is purely cosmetic, one edit is enough. If
the API shape changed — different argument order, a class split into two — the
surrounding code recipe likely needs rewriting too, not just the symbol. Read the new
symbol's signature before editing.

For wiki/core pages: a single-symbol rename can be patched in place. A whole-section
rewrite is `af_update_wiki`'s job — hand it off rather than doing partial section
rewrites here.
...
- **Whole-section wiki rewrites** → [`af_update_wiki.md`](./af_update_wiki.md). That
...
- **Missing libraries / import failures** → [`af_setup_environment.md`](./af_setup_environment.md).
...
- During an unrelated user task — like `af_update_wiki`, this should not be sneaked
...
7. Hand off any whole-section wiki rewrites to `af_update_wiki`.
...
9. Smoke-test affected scripts with `PYAUTO_TEST_MODE=1`.
10. Commit per the cadence the user picked (one file per commit, or batched).
