...
Where the user's own code lives and how it is wrapped: the likelihood entry point, the
`Analysis` class that calls it, and the script that composes the model. PyAutoFit fits
*your* code, so a session that cannot find it cannot resume.
...
re-derive it (a prior that collapses, a likelihood that returns `-inf` off-grid, a
config the cluster ignores).
